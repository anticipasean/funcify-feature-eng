...
internal class DefaultGraphQLSourceIndexFactory {
    ...
    class DefaultRootBase() : GraphQLSourceIndexFactory.RootBase {
        ...
        override fun fromRootDefinition(fieldDefinition: GraphQLFieldDefinition): ImmutableSet<GraphQLSourceIndex> {
            val convPathName = GraphQLSourceNamingConventions.getPathNamingConventionForGraphQLFieldDefinitions()
                    .deriveName(fieldDefinition)
            val convFieldName = GraphQLSourceNamingConventions.getFieldNamingConventionForGraphQLFieldDefinitions()
                    .deriveName(fieldDefinition)
            val sourcePath: SchematicPath = SchematicPathFactory.createPathWithSegments(pathSegments = persistentListOf(convPathName.qualifiedForm))
            when (val fieldType: GraphQLOutputType = fieldDefinition.type) {
                is GraphQLObjectType -> {
                    val graphQLSourceContainerType = GraphQLSourceContainerType(sourcePath = sourcePath,
                                                                                name = convFieldName,
                                                                                type = fieldType)
                    val graphQLSourceAttribute = GraphQLSourceAttribute(sourcePath = sourcePath,
                                                                        name = convFieldName,
                                                                        type = fieldType)
                    return persistentSetOf(graphQLSourceContainerType,
    ...
    class DefaultAttributeBase() : GraphQLSourceIndexFactory.AttributeBase {
        ...
        override fun forAttributePathAndDefinition(attributePath: SchematicPath,
                                                   attributeDefinition: GraphQLFieldDefinition): Option<GraphQLSourceContainerType> {
            when (val fieldType = attributeDefinition.type) {
                is GraphQLObjectType -> {
                    return GraphQLSourceContainerType(sourcePath = attributePath,
                ...
                is GraphQLList -> {
                    return if (fieldType.wrappedType is GraphQLObjectType) {
                        GraphQLSourceContainerType(sourcePath = attributePath,
                                                   name = GraphQLSourceNamingConventions.getFieldNamingConventionForGraphQLFieldDefinitions()
                                                           .deriveName(attributeDefinition),
                                                   type = fieldType).some()
                    } else {
    ...
    class DefaultParentDefinitionBase() : GraphQLSourceIndexFactory.ParentDefinitionBase {
        ...
        override fun withParentPathAndDefinition(parentPath: SchematicPath,
                                                 parentDefinition: GraphQLFieldDefinition): GraphQLSourceIndexFactory.ChildAttributeBuilder {
            return DefaultChildAttributeBuilder(parentPath = parentPath,
                                                parentDefinition = parentDefinition)
        }

    }

    class DefaultChildAttributeBuilder(private val parentPath: SchematicPath,
                                       private val parentDefinition: GraphQLFieldDefinition) : GraphQLSourceIndexFactory.ChildAttributeBuilder {

        override fun forChildAttributeDefinition(childDefinition: GraphQLFieldDefinition): GraphQLSourceAttribute {
            if (parentPath.pathSegments.isEmpty() || parentPath.pathSegments.lastOrNone()
                            .filter { s ->
                                s != GraphQLSourceNamingConventions.getPathNamingConventionForGraphQLFieldDefinitions()
            ...
            if (parentDefinition.type.toOption()
                            .filterIsInstance<GraphQLObjectType>()
                            .or(parentDefinition.type.toOption()
                                        .filterIsInstance<GraphQLList>()
                                        .map { gqll -> gqll.wrappedType }
                                        .filterIsInstance<GraphQLObjectType>())
            ...
            return GraphQLSourceAttribute(sourcePath = childPath,
