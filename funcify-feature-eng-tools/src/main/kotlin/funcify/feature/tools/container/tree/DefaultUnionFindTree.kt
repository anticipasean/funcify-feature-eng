 ...
 * @created 3/31/22
 */
internal data class DefaultUnionFindTree<P>(override val parents: PersistentMap<P, P> = persistentMapOf<P, P>(),
    ...
    override fun add(path: P): DefaultUnionFindTree<P> {
        return if (parents.containsKey(path)) {
            ...
            copy(parents = parents.put(path,
                                       path),
                 ranks = ranks.put(path,
                                   0),
                 treeSizes = treeSizes.put(path,
                                           1))
        }
    }
     ...
     * the current or _updated_ version of {@link UnionFindTreeCreator}
     */
    override fun find(path: P): Pair<Option<P>, UnionFindTree<P>> {
        /**
         * Case 0: Path does not have a listing in parents
         * => No _update_ (=> new instance with different parameters since immutable)
         * to the {@link UnionFindTreeCreator} should be made root_path is
         * empty in option returned
         */
        ...
        return path.some()
                .flatMap { currentPath: P ->
                    /**
                     * Find the root_path (<= the path in parent_listings
                ...
                .flatMap { rootAndCurrentPath: Pair<P, P> ->
                    ...
                    tailRecursionUsingOption(Pair(rootAndCurrentPath.second,
                        ...
                        if (currentPathAndParents.first != rootAndCurrentPath.first) {
                            currentPathAndParents.second[currentPathAndParents.first].toOption()
                                    .map { parent ->
                                        (parent to currentPathAndParents.second.put(currentPathAndParents.first,
                             ...
                             */
                            (currentPathAndParents.second).right()
                ...
                .let { rootAndUpdatedParents: Option<Pair<P, PersistentMap<P, P>>> ->
                     ...
                     */
                    rootAndUpdatedParents.map { rootAndParents ->
    ...
    override fun union(path1: P,
        ...
        val rootAndUnionFind2: Pair<Option<P>, DefaultUnionFindTree<P>> = rootAndUnionFind1.second.find(path2)
                .fold { pOpt, uft ->
                    Pair(pOpt,
                         ...
                         ?: DefaultUnionFindTree(parents = uft.parents.toPersistentMap(),
        ...
        return when {
            ...
            currentUnionFind.treeSizes[rootpath1]!! > currentUnionFind.treeSizes[rootpath2]!! -> {
                ...
                /**
                 * both the ranks and tree sizes of root_paths 1 and 2 are equal
