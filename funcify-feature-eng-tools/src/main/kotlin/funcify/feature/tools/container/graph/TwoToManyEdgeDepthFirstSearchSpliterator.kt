 ...
 * @created 4/2/22
 */
internal class TwoToManyEdgeDepthFirstSearchSpliterator<P, V, E>(
    ...
    override fun tryAdvance(action: Consumer<in Tuple5<V, P, E, P, V>>?): Boolean {
        ...
        val fullPathConnectionsForChildPath: List<Tuple5<V, P, E, P, V>> =
    ...
    private fun getFullPathConnectionForChildPath(childPath: P): List<Tuple5<V, P, E, P, V>> {
        return childToParentPathRelationships[childPath]
            .toOption()
            .flatMap { parentPath: P ->
                ...
                if (parentVertex != null && childVertex != null) {
                    val edges: ImmutableSet<E> =
                        edgesSetByPathPair[parentPath to childPath].toOption().getOrElse {
                            persistentSetOf()
                        }
                    if (edges.isNotEmpty()) {
                        edges
                            .stream()
                            .reduce(
                                mutableListOf<Tuple5<V, P, E, P, V>>(),
                                { l, e ->
                                    l.apply {
                                        add(
                                            Tuple5(
