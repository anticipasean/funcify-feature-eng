...
internal class TwoToOnePathsToEdgePathBasedGraphTest {
    ...
    interface MyInterface<T1, T2, T3> {
        ...
        operator fun get(t1: T1, t2: T2): T3?
    }

    class MyImpl<T1, T2, T3>(private val myMap: MutableMap<Pair<T1, T2>, T3>) :
        MyInterface<T1, T2, T3> {
        override fun get(t1: T1, t2: T2): T3? {
            return myMap[t1 to t2]
        }
    }

    @Test
    fun mapTest() {
        MyImpl<String, String, Int>(mutableMapOf<Pair<String, String>, Int>(("one" to "two") to 3))["one", "two"]
    }
    ...
    fun simpleMinimumSpanningTreeTest() {
        ...
        val graph: PathBasedGraph<Int, Int, Int> =
            edges.fold(
                vertices.fold(PathBasedGraph.emptyTwoToOnePathsToEdgeGraph()) { g, v ->
                    g.putVertex(v, v)
                }
            ) { g, e -> g.putEdge(e.first, e.second, e.third) }
        Assertions.assertEquals(
            22,
            graph.edgesAsStream().mapToInt { i -> i }.sum(),
            "not expected edge sum prior to minimizing"
        )
        val mstGraph: PathBasedGraph<Int, Int, Int> =
            graph.createMinimumSpanningTreeGraphUsingEdgeCostFunction(Comparator.naturalOrder())
        Assertions.assertEquals(
            14,
            mstGraph.edgesAsStream().mapToInt { i -> i }.sum(),
