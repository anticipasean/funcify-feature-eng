...
internal class TreeDepthFirstSearchSpliterator<T>(
    ...
    override fun tryAdvance(action: Consumer<in Pair<TreePath, T>>?): Boolean {
        ...
        val intermediateBuffer: Deque<Pair<TreePath, T>> = LinkedList()
        traversalFunction
            ...
            .forEachOrdered { p: Pair<TreePath, T> -> intermediateBuffer.offerFirst(p) }
        for (p: Pair<TreePath, T> in intermediateBuffer) {
            buffer.offerFirst(p)
        }
        if (buffer.isEmpty()) {
            expended = true
