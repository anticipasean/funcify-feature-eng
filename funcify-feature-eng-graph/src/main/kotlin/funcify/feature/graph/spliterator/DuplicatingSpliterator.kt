package funcify.feature.graph.spliterator
 ...
 * @created 3/19/22
 */
internal class DuplicatingSpliterator<T>(
    private val inputSpliterator: Spliterator<T>,
    private val bufferSupplier: () -> Deque<T> = { LinkedList<T>() },
    private val inputCharacteristics: Int = inputSpliterator.characteristics()
) : Spliterator<T> {
    ...
    companion object {
        ...
        private fun <T> createHasNextFunctionOnInputSpliterator(
            inputSpliterator: Spliterator<T>,
            parentBuffer: Deque<T>,
            childBuffers: List<Deque<T>>
        ): () -> Boolean {
            return {
                inputSpliterator.tryAdvance { t ->
                    parentBuffer.offer(t)
                    childBuffers.forEach { buff -> buff.offer(t) }
                }
            }
        }
     ...
     * - parentSplitr.trySplit() => parentSplitr1 [ A, B ] and parentSplitr2 [ C, D ] and
     * childSplitr [ C, D ] ?!?!
     * - an implementation of parentSplitr.trySplit() would need to take the inputSpliterator and
     * uses its trySplit call which typically, if implemented on that inputSpliterator, would return
     * a _new_ instance of Spliterator that references the values in the first half of the given
     * source index range and keep the other half in the inputSpliterator instance
     * - the childSplitr gets its values from the tryAdvance calls on the parent instance it was
     * created from
     * - thus, the childSplitr that was supposed to be a "duplicate" of the original parentSplitr
     * would end up only receiving the latter half of the values from the inputSpliterator instance
     *
     * Splits or parallelization should be done within a different context: upstream (before
     * creating the duplicatingspliterator) or downstream (on the output of the
     * duplicatingspliterator or its children)
     *
