 ...
 * @created 3/22/22
 */
internal class SlidingListWindowMappingSpliterator<T, R>(
    private val inputSpliterator: Spliterator<T>,
    private val windowMapper: (ImmutableList<T>) -> R,
    private val windowSize: Int = 1,
    private val characteristics: Int = DEFAULT_CHARACTERISTICS or inputSpliterator.characteristics()
) : Spliterator<R> {

    companion object {

        private const val DEFAULT_CHARACTERISTICS: Int = NONNULL or ORDERED
    ...
    /**
     * ## increment_size: ### Definition:
     * - the number of elements that can be viewed as "occurring after / succeeding" a given element
     * from a source sequence within a given window of size _n_
     *
     * ### Examples: #### Given sequence([ 1, 2, 3, 4, 5 ]) : the first window of size 3 => [ _ , 1,
     * 2 ]
     ...
     * - A 4 element window on the first element ( [0]: 1 ) of the sequence would be: <null>,
     * <null>, 1, 2
    ...
    private val projectedSize: Long =
        if ((characteristics and SIZED) == SIZED) {
            if ((targetWindowSize and 1) == 0) {
    ...
    override fun tryAdvance(action: Consumer<in R>?): Boolean {
        ...
        when (windowState) {
            /**
             * Case 1: The window is new so the input spliterator must be advanced forward until the
             * window reaches the increment_size
             */
            WindowState.NEW -> {
                var windowList = persistentListOf<T>()
                var advanceStatus: Boolean = false
                while (windowList.size < incrementSize &&
                    inputSpliterator.tryAdvance { t -> windowList = windowList.add(t) }.also {
                        advanceStatus = it
                    }) {}
            ...
            /**
             * Case 2: The window has reached the increment_size but has not yet reached _full_
             * target size and more elements are expected
             */
            WindowState.EXPANDING_PARTIAL -> {
                var windowList = window
                val advanceStatus: Boolean =
                    inputSpliterator.tryAdvance { t -> windowList = windowList.add(t) }
                if (advanceStatus) {
                    action.accept(windowMapper.invoke(windowList))
                    index++
                    if (windowList.size == targetWindowSize) {
            ...
            /**
             * Case 3: The window is _full_ size and each element retrieved from the input should be
             * appended to the window and the oldest or earliest element in the window should be
             * removed
             */
            WindowState.FULL -> {
                var windowList = window
            ...
            /**
             * Case 4: No more elements are left in input spliterator but there are remaining
             * windows to take action on before indicating the spliterator is closed
             */
