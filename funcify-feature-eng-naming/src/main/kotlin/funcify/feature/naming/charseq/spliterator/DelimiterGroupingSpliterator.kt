 ...
 * @created 3/14/22
 */
internal class DelimiterGroupingSpliterator(
    private val sourceSpliterator: Spliterator<Char>,
    ...
    private var delimitedQueue: Deque<Char> = LinkedList()
) : Spliterator<CharSequence> {

    companion object {
        // Not ordered (no clear comparison of charcontextgroups) or sized (unknown how many
        // charcontextgroups will be extracted from source)
        private const val DEFAULT_CHARACTERISTICS: Int = IMMUTABLE or NONNULL
    ...
    override fun tryAdvance(action: Consumer<in CharSequence>?): Boolean {
        ...
        /**
         * Case 1: A delimiter char group (sequence of delimiter characters) exists from a previous
         * advance on the source spliterator terminating at the start of a delimited group =>
        ...
        if (delimiterQueue.isNotEmpty()) {
            while (attemptToAdvance[0] && delimitedQueue.isEmpty()) {
                attemptToAdvance[0] =
                    sourceSpliterator.tryAdvance { c ->
        ...
        /**
         * Case 2: Next advance should yield the remaining characters of the current char group
         * terminating at the start of the next set of delimiter characters => Consume remaining
         * delimited group, empty delimited queue, and replace delimited queue
         */
        ...
        /**
         * Case 3: Consume the first group of characters either all delimiters or those surrounded
         * or terminated by a delimiter character => Consume first character and based on that
         * character, capture the first expected group type, empty that group type's queue, and
         * replace with new
         */
        ...
        while (attemptToAdvance[0] && !firstCharGroupTypeConsumed[0]) {
            ...
            firstCharGroupTypeConsumed[0] =
                delimiterQueue.isNotEmpty() && delimitedQueue.isNotEmpty()
        }
